...
粒子与 FOV 仅在 `ClientOverdriveState` 收到服务端确认的以下状态后运行：
...
| 预设 | 普通粒子每 tick 硬上限 | 速度线 | 音爆环 | FOV 因子 |
|---|---:|---|---|---:|
| PERFORMANCE | 8 | 关闭 | 关闭 | 0.7 |
| BALANCED | 20 | 开启 | 开启 | 1.0 |
| CINEMATIC | 32 | 开启 | 开启 | 1.2 |

Reduce Motion 将普通预算限制到 6，最多保留每侧一个翼尖点，关闭速度线和音爆环，并将预设 FOV 因子乘以 `0.35`。
...
保留原有平滑插值和停止后的回落。目标值为：

```text
原有速度增量 × presetFactor × reduceMotionFactor × fovIntensity
```

`fovIntensity` 裁剪到 `0.0–1.5`。零强度返回零额外 FOV。Mixin 仍只修改 `GameRenderer#getFov` 的临时返回值，不写入原版 FOV 选项，也不修改服务端状态。
