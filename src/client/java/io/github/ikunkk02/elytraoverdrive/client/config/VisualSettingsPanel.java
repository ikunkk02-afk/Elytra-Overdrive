...
public final class VisualSettingsPanel {
	...
	public static FlowLayout build(ConfigDraft draft, Runnable rebuild) {
		...
		LabelComponent intensityLabel = ControlPanelComponents.label(
				Component.translatable("screen.elytra_overdrive.visual.fov_intensity_value", draft.fovIntensity()),
				ControlTerminalTheme.TEXT
		);
		panel.child(intensityLabel);
		DiscreteSliderComponent intensity = Components.discreteSlider(Sizing.fill(100), 0.0, 1.5)
				.decimalPlaces(2)
				.snap(true)
				.setFromDiscreteValue(draft.fovIntensity());
		intensity.onChanged().subscribe(value -> {
			draft.fovIntensity(value);
			intensityLabel.text(Component.translatable(
					"screen.elytra_overdrive.visual.fov_intensity_value", draft.fovIntensity()
			));
		});
		panel.child(intensity);
		return panel;
	}
