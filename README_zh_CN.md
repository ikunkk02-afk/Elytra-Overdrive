...
`Performance`、`Balanced` 和 `Cinematic` 三种预设使用不同的粒子硬上限和 FOV 因子。各个效果可以单独关闭，Reduce Motion 会降低粒子与 FOV 强度。视觉设置不会改变服务端实际速度。
...
| 选项 | 默认值 | 说明 |
|---|---:|---|
| `playerSelectedMultiplier` | `2.0` | 玩家请求的飞行倍率，范围 `1.0–20.0`，之后仍受服务端上限限制。 |
| `enableHeldFireworkOverdrive` | `false` | 玩家是否请求使用手持烟花高速模式。 |
| `showHighSpeedParticles` | `true` | 本地高速粒子总开关。 |
| `enableHighSpeedFov` | `true` | 启用本地动态 FOV。 |
| `visualPreset` | `BALANCED` | 粒子预算和 FOV 预设：`PERFORMANCE`、`BALANCED` 或 `CINEMATIC`。 |
| `enableWingtipTrails` | `true` | 服务端确认高速飞行时显示翼尖气流。 |
| `enableSpeedLines` | `true` | 当前预设允许时显示外围速度线。 |
| `enableSonicBoomRing` | `true` | 当前预设允许时启用阈值触发的音爆环。 |
| `reduceMotion` | `false` | 降低动态粒子和 FOV，并关闭速度线与音爆环。 |
| `fovIntensity` | `1.0` | 额外 FOV 强度，范围 `0.0–1.5`。 |
...
| `serverMaximumMultiplier` | `10.0` | 服务端最终倍率上限，范围 `1.0–20.0`。 |
...
| `src/client/java` | 客户端界面、输入、粒子、视觉状态和 FOV 逻辑 |
